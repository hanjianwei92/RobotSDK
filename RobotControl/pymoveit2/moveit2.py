...
class MoveIt2:
    ...
    def __init__(
        ...
        # move_action goal handle
        self.__move_action_goal_handle : ClientGoalHandle = None
        self.__follow_joint_trajectory_action_client_goal_handle : ClientGoalHandle = None
    ...
    def move_to_pose(
        ...
        """
        if in_joint:
            target_joints = self.get_ik_result(position, quat_xyzw, target_link)
            self.move_to_joint(target_joints)
    ...
    def wait_until_executed(self)-> int|None:
        """
        ...
        return self.__executed_result
    
    def cancel_current_motion(self)-> bool|None:
        """
        Returns:
            bool: True if cancelled successfully, False failure 
        """
        if self.__execute_via_moveit:
            if self.__move_action_goal_handle is not None and self.__is_executing is True:
                return self.__move_action_goal_handle.cancel_goal()
            else:
                self._node.get_logger().warn("don`t need to cancel because of no executing action")
                return None
        else:
            if self.__follow_joint_trajectory_action_client_goal_handle is not None and self.__is_executing is True:
                return self.__follow_joint_trajectory_action_client_goal_handle.cancel_goal()
            else:
                self._node.get_logger().warn("don`t need to cancel because of no executing action")
                return None
    ...
    def get_robot_state(self)->bool:
        ...
        Returns:
            bool: True if robot is running, False otherwise
        """
        return self.__is_executing
    
    def get_robot_executed_result(self)->int:
        """
        get the result of robot execution

        Returns:
            int: -1 if robot execution is failed, 1 if robot execution is succeeded, 0 if robot executing
        """
        return self.__executed_result        
    ...
    def get_current_pose(self, link_name : Optional[str] = None) -> tuple[list[float], list[float]]:
        """
        get current cartesian pose of specified link
        Args:
            link_name (Optional[str], optional):specified link name, if is none, defaults to end-effector link.

        Returns:
            tuple[list[float], list[float]]: specified link`s cartesian position(xyz) and orientation_quaternion(xyzw)
        """
        fk_link_names = [link_name] if link_name is not None else [self.__end_effector_name]
        pose = self.compute_fk(fk_link_names=fk_link_names)[0].pose
        return [pose.position.x,pose.position.y, pose.position.z], \
                [pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w]
                
    def get_current_joints(self)->list[float]:
        """
        get current activate joints radian value  
        Returns:
            list[float]:  joints radian value  
        """
        joint_names = self.__joint_state.name
        joint_value = self.__joint_state.position 
        return [joint_value[joint_names.index(joint)] for joint in self.__joint_names]
    
    def get_fk_result(self, joints: list[float], fk_link_name: Optional[str]=None)-> Optional[tuple[list[float], list[float]]]:
        """
        get compute forward kinematics result
        Args:
            joints (list[float]): target joint value
            fk_link_name (Optional[str], optional): specified link name, if is none, defaults to end-effector link.
        Returns:
            tuple[list[float], list[float]]: specified link`s cartesian position(xyz) and orientation_quaternion(xyzw)
        """
        fk_link_names = [fk_link_name] if fk_link_name is not None else [self.__end_effector_name]
        res = self.compute_fk(joint_state = joints, fk_link_names=fk_link_names)
        if res is None: 
            return None
        pose = res[0].pose
        return [pose.position.x,pose.position.y, pose.position.z], \
    ...
    def get_ik_result(self, pos: list[float], quat: list[float], 
                      target_link: str = None, start_joint_state: Optional[List[float]]=None)->Optional[list[float]]:
        """
           get inverse kinematics for the given pose and target_link
        Args:
            pos (list[float]): target link`s position
            quat (list[float]): target link quaternion
            target_link (str, optional): target link.

        Returns:
            list[float]: activate joints radian values
        """
        res =  self.compute_ik(pos, quat, target_link, start_joint_state)
        if res is None:
            return None
        joint_v = res.position
        joint_n = res.name
        joint_normal_value = [joint_v[joint_n.index(joint)] for joint in self.__joint_names]

        return joint_normal_value
    
    def compute_ik(
        self,
        position: Union[Point, Tuple[float, float, float]],
        quat_xyzw: Union[Quaternion, Tuple[float, float, float, float]],
        ik_target_link_name: Optional[str] = None,
        start_joint_state: Optional[Union[JointState, List[float]]] = None,
        ...
        self.__compute_ik_req.ik_request.ik_link_name = self.__end_effector_name if ik_target_link_name is None else ik_target_link_name
    ...
    def __response_callback_move_action(self, response):
        ...
        self.__move_action_goal_handle = goal_handle
    ...
    def __response_callback_follow_joint_trajectory(self, response):
        ...
        self.__follow_joint_trajectory_action_client_goal_handle = goal_handle
        self.__is_executing = True
        self.__is_motion_requested = False
    ...
    @property 
    def plannerID(self)->str:
        
        return self.__move_action_goal.request.planner_id
    
    @plannerID.setter
    def plannerID(self, value:str):
